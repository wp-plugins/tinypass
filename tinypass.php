 ...
 */
$tinypass_post_req = null;
$tinypass_tag_req = null;
$tinypass_meter = null;
...
function tinypass_intercept_content($content) {
	...
	global $tinypass_post_req;
	global $tinypass_tag_req;
...
//      $cookieName = "tr_" . preg_replace('/[^0-9]*/', '', $tagOptions->getResourceId());
	...
	if ($postOffer) {
		...
		$tinypass_post_req = array('req' => $req,
	...
	if ($tagOffer) {
		...
		$tinypass_tag_req = array('req' => $req2,
	...
	if ($tagOptions->isPostFirstInOrder() == false) {
		$temp = $tinypass_post_req;
		$tinypass_post_req = $tinypass_tag_req;
		$tinypass_tag_req = $temp;
	}
...
function tinypass_append_ticket($content) {
	...
	if ($tinypass_post_req == null && $tinypass_tag_req == null)
		return $content;

	$tout = '';

	if ($tinypass_post_req != null && $tinypass_tag_req) {

		$request1 = $tinypass_post_req['req'];
		$request2 = $tinypass_tag_req['req'];
		...
		$message1 = stripslashes($tinypass_post_req['message1']);
		$message2 = stripslashes($tinypass_tag_req['message1']);

		$sub1 = stripslashes($tinypass_post_req['sub1']);
		$sub2 = stripslashes($tinypass_tag_req['sub1']);
		...
		$request = $tinypass_tag_req;
		if ($tinypass_post_req != null)
			$request = $tinypass_post_req;
